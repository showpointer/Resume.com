...
<body bgcolor='white'>
<table border='1' width='519' celpadding='0' cellspacing='0' align='center'>
  <tr>
	<td width='150' align='right' valign='middle'><a href="View.do?month=<%=currMonth%>&year=<%=currYear%>&action=0"><font size="3">Previous Month</font></a></td>
	<td width='260' align='center' valign='middle'><b><font size="4"><%=getDateName (cal.get(cal.MONTH)) + " " + cal.get(cal.YEAR)%></font></b></td>
	<td width='173' align='left' valign='middle'><a href="View.do?month=<%=currMonth%>&year=<%=currYear%>&action=1"><font size="3">Next Month</font></a></td>
  </tr>
	</table>
<table border="0" width="520" bordercolorlight="#C0C0C0" bordercolordark="#808080" style="border-collapse: collapse" bordercolor="#111111" cellpadding="0" cellspacing="0" align='center'>
  <td width="100%">
...
<%
			...
			else
			{
				...
				pageContext.setAttribute("year", cal.get(cal.YEAR));
				pageContext.setAttribute("month", cal.get(cal.MONTH));
				pageContext.setAttribute("day", dispDay);
%> 
		<td bgcolor ="<%=todayColor%>" width="<%=boxSize%>" align="left" height="<%=boxSize%>" valign="top">
			<%=dispDay%>
		<br>
		<c:forEach var="recruit" items="${recruit}">
			<c:if test="${fn:substring(recruit.startedDate,0,4) eq pageScope.year && fn:substring(recruit.startedDate,6,7) eq pageScope.month+1 && fn:substring(recruit.startedDate,8,10) eq pageScope.day }">
			  <a href='javascript:OpenPopUp(${recruit.rno})'>[시작]${recruit.company}/${recruit.job}</a>
			  <br>
			</c:if>
			<c:if test="${fn:substring(recruit.startedDate,0,4) eq pageScope.year && fn:substring(recruit.endedDate,6,7) eq pageScope.month+1 && fn:substring(recruit.endedDate,8,10) eq pageScope.day }">
			   <a href='javascript:OpenPopUp(${recruit.rno})'>[끝]${recruit.company}/${recruit.job}</a>
			  <br>
			</c:if>
		</c:forEach>
		</td>
